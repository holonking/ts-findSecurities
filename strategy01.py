...
class strategy_day_01:
        ...
        for i in range(40, len(df.close)):
            ...
            if con:
                ...
                if trade_state == 'holding':
                    ...
                    if log:
                        print(
                        "%s traded, profit: %8.2f | total %3d trades, won:%8.2f  lost :%8.2f  accuracy:%3.1f%% | total profit:%10.2f  %3.1f%% " % (
                        sec_num, profit_t, trade_count, gain, lose, accuracy, profit, returnrate))
    ...
    def run(self, sec_num, draw=True, log=True, df=None, start='2015-04-01', end='2016-04-01'):
        ...
        if cost > 0:
            ...
            if log:
                print("Total %d trades on %s, a:%3.2f%%, p%8.2f|r%3.2f%%  " % (
                len(trades), sec_num, accuracy, profit, profit_rate))
        else:
